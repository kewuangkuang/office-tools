...
- `index.html`：唯一应用入口和唯一业务代码来源，通常必须改。
- `excel-tools.html`：旧本地入口兼容页，只跳转到 `index.html`，不要在这里加入业务代码。
...
2. 这个项目是单文件前端应用，只有 `index.html` 含内嵌第三方库。不要阅读文件开头的大段内嵌库；优先用关键词定位业务代码。
3. 改业务逻辑时只修改 `index.html`；用测试保证 `excel-tools.html` 仍是无业务代码的跳转页。
4. 能做小改就小改，不重构成框架项目，除非用户明确要求。
5. 默认本地处理；PDF 回单模式和批量识别命名允许按“文字层 → 百度 OCR → Tesseract”调用，并允许配置百度 API 凭证。
...
- 如果只是导出格式小改，至少确认相关函数和导出路径在 `index.html` 内一致，并运行回归测试。
